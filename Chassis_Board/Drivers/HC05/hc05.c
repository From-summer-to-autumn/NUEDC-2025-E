...
void BLE_Send_Bit(unsigned char ch)
{
    ...
    while( DL_UART_Main_isBusy(HC_05_INST) == true );
    //发送单个字符
    DL_UART_Main_transmitData(HC_05_INST, ch);
}
...
{
    //清除串口中断标志
    NVIC_ClearPendingIRQ(HC_05_INST_INT_IRQN);
    //使能串口中断
    NVIC_EnableIRQ(HC_05_INST_INT_IRQN);
...
{
    //如果产生了串口中断
    switch( DL_UART_getPendingInterrupt(HC_05_INST) )
    {
        case DL_UART_MAIN_IIDX_RX://如果是接收中断
            ...
            {
                BLERX_BUFF[BLERX_LEN++] = DL_UART_Main_receiveData(HC_05_INST); // 接收数据
            }
            else
            {
                uint8_t temp = DL_UART_Main_receiveData(HC_05_INST); // 不保存数据
            }
